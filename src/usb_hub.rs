...
impl OverloadedPort {
    fn from_currents(total: f32, port1: f32, port2: f32, port3: f32) -> Option<Self> {
        let total_overload = total > MAX_TOTAL_CURRENT * CURRENT_MARGIN;
        let port1_overload = port1 > MAX_PORT_CURRENT * CURRENT_MARGIN;
        let port2_overload = port2 > MAX_PORT_CURRENT * CURRENT_MARGIN;
        let port3_overload = port3 > MAX_PORT_CURRENT * CURRENT_MARGIN;
        ...
        if total_overload {
            Some(Self::Total)
        } else if port1_overload {
            Some(Self::Port1)
        } else if port2_overload {
            Some(Self::Port2)
        } else if port3_overload {
            Some(Self::Port3)
        } else {
            None
        }
